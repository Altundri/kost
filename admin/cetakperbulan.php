...
include "koneksi.php";
$bln_ini = date("Y-m");
$sql= mysqli_query($con, "SELECT DISTINCT *  FROM pelanggan INNER JOIN transaksi ON pelanggan.id_pelanggan = transaksi.id_pelanggan INNER JOIN konfirmasi ON transaksi.id_transaksi = konfirmasi.id_transaksi WHERE konfirmasi.status='Y' && transaksi.tgl_masuk LIKE '%$_GET[bln_ini]%' ");
$no=1;
$total=0;
